...
public class EmailSchedulingServiceImp implements IEmailScheduling {
	...
	public ResponseEntity<EmailResponse> scheduleEmail(String email, String username, event e) {
		...
		String StringDate = dateFormat.format(e.getEventDateEnd());
		...
		try {
			...
			EmailResponse emailResponse = new EmailResponse(true, jobDetail.getKey().getName(),
					jobDetail.getKey().getGroup(), "Email Scheduled Successfully :D");
	...
	private Trigger buildTrigger(JobDetail jobDetail, ZonedDateTime startAt) {
		return TriggerBuilder.newTrigger().forJob(jobDetail)
				.withIdentity(jobDetail.getKey().getName(), "email-triggers").startAt(Date.from(startAt.toInstant()))
